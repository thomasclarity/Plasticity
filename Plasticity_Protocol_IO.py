...
Function: Sends pulses to the photometry system and camera while sending stimulation to the animal during an IO day of
the Plasticity study.
    ...
    3. IO day stimulation paradigm (32 minutes)
...
Code adapted from justphotometry.py and Photometry_PulseFreqv2.py by RMikofsky
...
msg_protocol = input('Type protocol (ex. "IO" or "Plasticity" - for this code it should be "IO") and hit enter. \n')
...
while protocolinputtest == 'false':
    if msg_protocol == 'Plasticity':
        print("This is not the Plasticity protocol. Quit this protocol and open Plasticity40Hz")
        ...
        msg_protocol = input(
            'Type protocol (ex. "IO" or "Plasticity" - for this code it should be "IO") and hit enter. \n')
...
sendtimestamp(json.dumps('Photometry recording IO experiment'))
...
base_start = {'stim_event': 'baseline', 's_or_e_of_event': 'start', 'event_type': 'IO',
...
base_end = {'stim_event': 'baseline', 's_or_e_of_event': 'end', 'event_type': 'IO',
                   'pulse_width': 'NA', 'pulse_frequency': 'NA', 'pulse_n': 'NA',
                   'pulse_laserpower': 'NA',
                   'mouse': msg_mouse, 'protocol': msg_protocol, 'week': msg_week,
                   'dayinweek': msg_dayinweek, 'fileid': fileid, 'plasticitystim': msg_plasticitystim,
...
sendtimestamp(json.dumps('Photometry recording IO Baseline'))
